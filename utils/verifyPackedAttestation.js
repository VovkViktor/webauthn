...
let verifyPackedAttestation = (webAuthnResponse) => {
  ...
  if (attestationStruct.attStmt.x5c) {
    ...
    const x = pubKeyCose.get(-2);
    const y = pubKeyCose.get(-3);
    const publicKey = Buffer.concat([tag, x, y]);
    ...
    if (pubKeyCose.get(COSEKEYS.kty) === COSEKTY.EC2) {
      ...
      console.log("pubKeyCose.get(COSEKEYS.e)", pubKeyCose.get(COSEKEYS.e));

      key.importKey(
        {
          n: pubKeyCose.get(COSEKEYS.n),
          e: pubKeyCose.get(COSEKEYS.e), //65537,
        },
        "components-public"
      );
      ...
      return {
        ...
        authrInfo: {
          fmt: "packed",
          publicKey: base64url(key.exportKey("pkcs1-public-pem")),
          counter: authDataStruct.counter,
          credID: base64url(authDataStruct.credID),
        },
      };
