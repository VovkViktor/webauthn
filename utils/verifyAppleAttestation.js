...
let asn1ObjectToJSON = (asn1object) => {
  ...
  if (!asn1object.sub) {
    if (
      asn1object.typeName() === "BIT_STRING" ||
      asn1object.typeName() === "OCTET_STRING"
    )
      JASN1.data = asn1object.stream.enc.slice(
        asn1object.posContent(),
        asn1object.posEnd()
      );
...
let containsASN1Tag = (seq, tag) => {
  for (let member of seq) if (member.type === "[" + tag + "]") return true;
...
let verifyAppleAnonymousAttestation = (webAuthnResponse) => {
  ...
  if (!AppleNonceExtension)
    throw new Error(
      "The certificate is missing Apple Nonce Extension 1.2.840.113635.100.8.2!"
    );
  ...
  let certificateNonceBuffer =
    appleNonceExtensionJSON[1].data[0].data[0].data[0].data;
